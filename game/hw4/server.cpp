...
int main()
{
	...
	while(window->isOpen()) {
		...
		bool respawn = window->hasFocus() && sf::Keyboard::isKeyPressed(sf::Keyboard::R);

		if(respawn) {
			EventManager::getInstance()->raise(Event(EventType::SPAWN));
		}
		...
		if(clients > 0) {
			...
			serverUpdate_j["globalRespawnEvent"] = respawn;
